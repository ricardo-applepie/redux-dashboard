    ...
    <div className="popup">
      ...
      <div className="popup__wrapper">
        <div className="popup-fields">
          <div>
            <SearchBar
              onChangeHandle={onChangeInputHandle}
              inputVal={input}
              inputType="search"
            ...
            {tabs.map(function (tab, index) {
              return (
                <div
                  ...
                  className={`popup-toggle-btn__fields  ${
                    selectedIndex == index ? 'active-btn' : ''
                  }`}
                >
                  <p className="popup-toggle-btn-text">{tab}</p>
                </div>
              );
            })}
          </div>
          <div>
            <div className="popup-sub__wrapper">
              ...
              <div>
                <ul
                  className={` "popup-fields__list  "  ${
                    selectedIndex == 0 ? 'selectedField' : 'hidetab'
                  }`}
                >
                      ...
                      <React.Fragment>
                        <li key={field.id} className="popup-fields__list-item">
                          <input
                            ...
                            className="popup-fields__list-item__input"
                            type="checkbox"
                          />
                          <span className="popup-fields__list-item__label">
                            {field.icon}{' '}
                          </span>
                          <p className="popup-fields__list-item__text">
                            {field.name}{' '}
                          </p>
                        </li>
                      </React.Fragment>
                ...
                <ul
                  className={` "popup-fields__list  "   ${
                    selectedIndex == 1 ? 'selectedField' : 'hidetab'
                  }`}
                >
                  tab 2
                </ul>
                <ul
                  className={` "popup-fields__list  "   ${
                    selectedIndex == 2 ? 'selectedField' : 'hidetab'
                  }`}
                >
                  tab 3
                </ul>
      ...
      <div className="popup-button__wrapper button">
        <Button primaryButtonColor="button-primary">Done</Button>

        <Button primaryButtonColor="button-normal">Cancel</Button>
      </div>
    </div>
  );
}
